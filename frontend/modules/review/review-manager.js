...
class ReviewManager {
    ...
    async startSession(sessionType = 'mixed') {
        try {
            console.log(`[Review] Starting ${sessionType} session for user ${this.userId}`);
    ...
    async submitAnswer(quality) {
        ...
        try {
            ...
            console.log(`[Review] Answer recorded. Correct: ${this.sessionStats.correctAnswers}, Streak: ${this.sessionStats.streak}`);
    ...
    startTimer() {
        ...
        this.timerInterval = setInterval(() => {
            if (!this.isPaused) {
                this.sessionStats.totalTime = Math.floor((Date.now() - this.sessionStats.startTime) / 1000);
                this.updateSessionStats();
            }
        }, 1000);
    }
    ...
    endSession(summary) {
        ...
        if (!summary) {
            summary = {
                ...
                accuracy: this.sessionStats.cardsReviewed > 0
        ...
        // Fire custom event
        window.dispatchEvent(new CustomEvent('reviewSessionEnded', {detail: summary}));
    }
    ...
    showCompletionScreen(summary) {
        ...
        const duration = summary.duration_minutes || Math.floor(this.sessionStats.totalTime / 60);
    ...
    reset() {
        ...
        if (container) {
            ...
            document.getElementById('mixed-btn')?.addEventListener('click', () => this.startSession('mixed'));
            document.getElementById('new-btn')?.addEventListener('click', () => this.startSession('new'));
            document.getElementById('review-btn')?.addEventListener('click', () => this.startSession('review'));
        }
    }
